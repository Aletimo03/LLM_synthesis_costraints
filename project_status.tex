...
A correction loop is implemented (synthesis-failure trigger); extending it to fire on coverage/timing failures is a planned next step. The framework also supports \emph{multiple prompt versions} (\texttt{v1\_base}, \texttt{v2\_base}) and a \texttt{-{}-compare-prompts} mode that runs every prompt on each design/seed so their results can be compared directly.

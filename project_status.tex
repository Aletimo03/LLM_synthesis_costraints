...
\end{enumerate}
\subsection*{A note on coverage completeness}
The ten runs currently labeled \emph{accepted} must be interpreted with care.
Post-hoc inspection of the OpenSTA logs reveals that every one of them was
analyzed exclusively on register-to-register paths: the input-to-register and
register-to-output path groups were entirely absent, because the generated SDC
omitted the \texttt{-clock} association required for \texttt{set\_input\_delay}
and \texttt{set\_output\_delay} to take effect.
OpenSTA silently skips unconstrained ports rather than raising an error, so the
tool reported clean timing on the only paths it could see, while the
input and output timing environments were never exercised.
